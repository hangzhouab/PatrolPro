...
import com.ab.health.contactstview.SideBar.OnTouchingLetterChangedListener;
...
public class MainActivity extends Activity {
	...
	private View viewGongGao, activityServer, activityTool, activityTXL;
	...
	private TextView titleBar, btn_loginout;
	...
	private void InitGongGaoViewFliper() {
		...
		gonggaoLV.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
					if (gonggaoLV.getLastVisiblePosition() == (gonggaoLV
							.getCount() - 1)) {
		...
		gongGaoAdapter = new SimpleAdapter(this, gonggaoData,
				R.layout.view_gonggao_list, new String[] { "title", "date" },
				new int[] { R.id.view_meal_list_name_tv,
						R.id.view_meal_list_cal_tv });
	...
	private void InitPatrolRecordViewFliper() {
		...
		patrolRecordLV.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
					if (patrolRecordLV.getLastVisiblePosition() == (patrolRecordLV
							.getCount() - 1)) {
						patroltouch = true;
		...
		patrolRecordAdapter = new NFCAdapter(this, patrolRecordData,
				R.layout.view_patrol_record_list, new String[] { "address",
						"time" }, new int[] { R.id.view_meal_list_name_tv,
						R.id.view_meal_list_cal_tv });
	...
	private void InitButton() {
		...
		activityTool = flater.inflate(R.layout.activity_patrol_record,
				contentViewPager);
		viewGongGao = flater.inflate(R.layout.activity_gonggao2,
				contentViewPager);

		// activityPhysiology = flater.inflate(R.layout.fragment_patrol_query,
		// contentViewPager);
		activityTXL = flater.inflate(R.layout.activity_txl, contentViewPager);
		initTXLViews();
	...
	private class OnClickListener implements View.OnClickListener {
		...
		public void onClick(View arg0) {
			...
			case R.id.bottombar_bbs:
				...
				btn_loginout.setVisibility(View.GONE);
				LoadAddressListAysnTask loadAddresslist = new LoadAddressListAysnTask();
				loadAddresslist.execute(0);
				break;
			...
			case R.id.topbar_loginout:
				SharedPreferences appSetting = getSharedPreferences(
						AppSetting.getSettingFile(), MODE_PRIVATE);
				Editor editor = appSetting.edit();
				editor.putBoolean(AppSetting.isRegister, false);
				editor.commit();
	...
	private class LoadGongGaoAysnTask extends
		...
		protected void onPreExecute() {
			...
			if (isComplelet) {
				Toast.makeText(getApplicationContext(), "全部信息已加载",
		...
		protected Integer doInBackground(Object... params) {
			...
			gonggaoParam = "?startid=" + sId + "&endid=10&unit="
					+ ramOrgniztion;
	...
	private class LoadPatrolRecordAysnTask extends
			AsyncTask<Object, Integer, Integer> {
		...
		protected void onPreExecute() {
			if (isComplelet) {
				Toast.makeText(getApplicationContext(), "全部信息已加载完成",
						Toast.LENGTH_SHORT).show();
			} else {
				Toast.makeText(getApplicationContext(), "正在加载，请稍后...",
						Toast.LENGTH_SHORT).show();
			}
			if (!patroltouch) {
		...
		protected Integer doInBackground(Object... params) {
			int sId = (Integer) params[0];
			loadParam = "?username=" + ramUsername + "&startid=" + sId
					+ "&endid=10" + "&unit=" + ramOrgniztion;
	...
	private class UploadPatrolRecordAysnTask extends
			AsyncTask<Object, Integer, Integer> {
		...
		protected void onPreExecute() {
			...
			uploadparam = "?username=" + ramUsername + "&address=" + TAGaddress
					+ "&tagid=" + tagid + "&unit=" + ramOrgniztion;
	...
	private class CreateTableAysnTask extends
			AsyncTask<Object, Integer, Integer> {
		int ret;
		String url, param;
	...
	private class LoadAddressListAysnTask extends
			AsyncTask<Object, Integer, Integer> {
		String loadParam, url;
		...
		protected void onPostExecute(Integer result) {
			...
			SourceDateList = filledData(addressListData);
	...
	private Integer AddressListJsonHandle(String retResponse) {
		...
		try {
			...
			for (int i = 0; i < foodsArray.length(); i++) {
				...
				String zhiwu = temp.getString("zhiwu");
				String bumen = temp.getString("bumen");
				String shortphone = temp.getString("shortphone");
				courseItem.put("username", username);
				courseItem.put("phone", phone);
				courseItem.put("zhiwu", zhiwu);
				courseItem.put("bumen", bumen);
				courseItem.put("shortphone", shortphone);
	...
	private List<SortModel> filledData(List<HashMap<String, String>> txl) {
		List<SortModel> mSortList = new ArrayList<SortModel>();

		for (int i = 0; i < txl.size(); i++) {			
			SortModel sortModel = new SortModel();
			sortModel.setName(txl.get(i).get("username"));
			sortModel.setPhone(txl.get(i).get("phone"));
			sortModel.setBumen(txl.get(i).get("bumen"));
			sortModel.setZhiwu(txl.get(i).get("zhiwu"));
			sortModel.setShortphone(txl.get(i).get("shortphone"));
			
			String pinyin = characterParser.getSelling(txl.get(i).get("username"));
